...
class LanguageAugEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.num_l_layers = config.num_l_layers
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.lang_aug_encoder = LanguageAugEncoder(config)

        self.img_embeddings = ImageEmbeddings(config)

        self.knowledges_text_encoder = CrossmodalEncoder(config)  
        ...
        if config.fix_lang_embedding or config.fix_local_branch:
            ...
            for k, v in self.lang_encoder.named_parameters():
                v.requires_grad = False
            for k, v in self.lang_aug_encoder.named_parameters():
                v.requires_grad = False
    ...
    def forward_text(self, txt_ids, txt_masks, knowledge_fts ,act_txt=None, act_txt_masks=None, obj_txt=None, obj_txt_masks=None):
        ...
        # print(f'11111111111111111{knowledge_fts}')
        ...
        obj_embeds = self.lang_encoder(obj_embeds,obj_txt_masks)
        
        extended_obj_masks = extend_neg_masks(obj_txt_masks)

        knowledges_embeds = self.knowledge_proj(knowledge_fts.view(batch_size, -1, 512))

        for layer_module in self.knowledges_text_encoder.x_layers:
            obj_knowledges_embeds = layer_module.forward_lang2visn(
                obj_embeds, extended_obj_masks,
        ...
        for layer_module in self.knowledges_text_encoder.x_layers:
            txt_knowledges_embeds = layer_module.forward_lang2visn(
                txt_embeds, extended_txt_masks,
                obj_knowledges_embeds, None,
            )
        ...
        txt_knowledges_embeds = 0.2*txt_knowledges_embeds + 0.8*txt_embeds

        # print(f'aug_weight_txt_knowledges_embeds{aug_weight}')

        txt_embeds = txt_knowledges_embeds
        obj_embeds = obj_knowledges_embeds
...
# 这个 forward_navigation_per_step 方法的主要功能是处理基于文本和视觉信息的导航任务，其中包含全局和局部分支的计算，融合全局与局部信息，并最终产生用于导航决策的 logits
    def forward_navigation_per_step(
        ...
        vp_obj_embeds = self.local_encoder.encoder(obj_embeds,None, vp_obj_embeds, vp_obj_masks)
        # 强调 vp_embeds 的权重
        ...
        #     aug_weight = self.instr_sigmoid(aug_linear_weight+ori_linear_weight)
        #     vp_embeds = torch.mul(aug_weight,instr_aug_embeds) + torch.mul((1-aug_weight),vp_embeds)  
        vp_embeds= 0.8 * vp_embeds + 0.2 * vp_obj_embeds 
